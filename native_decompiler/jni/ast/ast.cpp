...
void Ast::check_special_number(Expression* const& expression, const bool& isCdata) {
	const uint64_t rawDouble = *reinterpret_cast<const uint64_t*>(&expression->constant->number);
...
Ast::CONSTANT_TYPE Ast::get_constant_type(Expression* const& expression) {
	static const auto is_valid_number_constant = [](const double& number)->bool {
		const uint64_t rawDouble = *reinterpret_cast<const uint64_t*>(&number);
...
Ast::Expression* Ast::new_signed_literal(const uint16_t& signedLiteral) {
	Expression* const expression = new_expression(AST_EXPRESSION_CONSTANT);
	expression->constant->type = AST_CONSTANT_NUMBER;
	int16_t signedValue = static_cast<int16_t>(signedLiteral);
	expression->constant->number = static_cast<double>(signedValue);
	return expression;
}
...
Ast::Expression* Ast::new_number(const Function& function, const uint16_t& index) {
	...
	case Bytecode::BC_KNUM_INT: {
		uint32_t intValue = function.get_number_constant(index).integer;
		int32_t signedValue = static_cast<int32_t>(intValue);
		expression->constant->number = static_cast<double>(signedValue);
		break;
	}
	case Bytecode::BC_KNUM_NUM: {
		uint64_t numValue = function.get_number_constant(index).number;
		expression->constant->number = *reinterpret_cast<const double*>(&numValue);
...
Ast::Expression* Ast::new_table(const Function& function, const uint16_t& index) {
	const auto new_table_constant = [this](const Bytecode::TableConstant& constant)->Expression* {
		...
		case Bytecode::BC_KTAB_INT:
			expression->constant->type = AST_CONSTANT_NUMBER;
			if (constant.integer > 0x7FFFFFFF) {
				int32_t signedValue = static_cast<int32_t>(constant.integer);
				expression->constant->number = static_cast<double>(signedValue);
			} else {
				expression->constant->number = static_cast<double>(constant.integer);
			}
			break;
		case Bytecode::BC_KTAB_NUM:
			expression->constant->type = AST_CONSTANT_NUMBER;
			expression->constant->number = *reinterpret_cast<const double*>(&constant.number);
			check_special_number(expression);
			break;
...
Ast::Expression* Ast::new_cdata(const Function& function, const uint16_t& index) {
	...
	case Bytecode::BC_KGC_I64:
		...
		expression->constant->signed_integer = *reinterpret_cast<const int64_t*>(&function.get_constant(index).cdata);
		break;
	...
	case Bytecode::BC_KGC_COMPLEX:
		...
		expression->constant->number = *reinterpret_cast<const double*>(&function.get_constant(index).cdata);
